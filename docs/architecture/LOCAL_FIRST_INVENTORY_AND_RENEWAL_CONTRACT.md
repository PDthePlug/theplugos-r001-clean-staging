...
Card, QR, cash capture, refunds, stock receipt, waste, adjustment, BOM, and
financial postings are deliberately outside this slice. They must not be
represented as successful by the UI or projected as if this contract had
settled them.
...
`inventory_movements` row for every replicated reservation or release. A
movement contains the immutable Hub event ID, business, branch, product,
order, movement type, signed quantity delta, balance-before, balance-after,
...
an approved inventory-receive/allocation command exists. Copying one global
legacy quantity into multiple branch balances would fabricate stock, so the
migration deliberately fails safe instead.
